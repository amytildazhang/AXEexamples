...
ll_norm <- function(beta, mu, Sigma_inv, ...) {
    if ( length(Sigma_inv) > 1) {
        ...
        as.vector(det - length(beta)/2 * log(2*pi) - t(dif) %*% Sigma_inv %*% dif/2)
...
lpl_approx_pois <- function(y, X, beta, Sigma_inv, mu, tau_inv = NULL, dist = "poisson") {
    ...
    inits <- beta
    mode <- optim(inits, model_fun, control = list(fnscale = -1),
                  gr = grad_fun,
                  y = y, Sigma_inv = Sigma_inv, X = X, mu = mu, tau_inv = tau_inv,
                  method = ifelse(length(beta > 1), "BFGS", "Brent"))

    list(
        ll =  .lpl_approx_pois(beta0 = mode$par, X = X, Sigma_inv = Sigma_inv, y = y, mu = mu),
        beta = mode$par
    )
...
.lpl_approx_norm <- function(beta0, Sigma_inv, X, y, mu, tau_mat = NULL, tau_inv = NULL) {
    if ( is.null(tau_mat) ) {
...
lpl_approx_norm <- function(y, X, beta, Sigma_inv, mu, tau_inv = NULL, dist = "normal") {
    ...
    model_fun <- function(pars, y, Sigma_inv, X, tau_inv, ...) {
        eta = X %*% pars
        sum(ll_norm(pars, 0, Sigma_inv)) + sum(ll_norm(beta = y, mu = eta, Sigma_inv = tau_inv))
    }

    grad_fun <- function(pars, y, Sigma_inv, X, tau_mat = NULL, tau_inv = NULL, ...) {
        if ( is.null(tau_mat) ) {
            as.vector(gr_norm(pars, Sigma_inv) + 2 * tau_inv * t(X) %*% (y - X %*% pars))

        } else {
            as.vector(gr_norm(pars, Sigma_inv) + 2 * t(X) %*% tau_mat %*% (y - X %*% pars))
        }
    }
