...
axe_radon_simul <- function(info = radon_2, posteriors = radon_2$posteriors,
    ...
    loop_over_radon_simul(
        ...
        FUN = function(mod_no, dat, n, perc, subset_idx, info, seed, var, posteriors) {
            ...
            dperc <- perc;
            vars <- dplyr::filter(posteriors, iter == subset_idx, perc == dperc,
            ...
            if (var == "axe") {
                tau2 <- vars$tau2hat_post; sigma2 <- vars$sig2hat_post
            } else if (var == "iis") {
                sigma2 <- vars$sig2hat_iis_fm; tau2 <- (vars$tau2hat_iis_fm)

            } else if (var == "map") {
                tau2 <- vars$tau2hat_map; sigma2 <- vars$sig2hat_map;
...
axe_air <- function(info = air$data, posteriors = air$posteriors) {
    ...
    rho_s <- posteriors$rho_s; rho_t <- posteriors$rho_t; tau2 <- posteriors$tau2
