...
#   psis_yhat_lco <- loo::E_loo(yhats[fin_mask, ], psis_obj_lco,
#     type = "mean",
...
pfit_eight <- function(info = eight, seed = 238972, nodes = 6, ...) {
  purrr::map((info$data_scale), function(scl) {
    ...
    # fit to full data
    fit_sch8 <- rstan::sampling(stanmodels$eightschools_sim,
      data = scld_dat,
      seed = seed, refresh = 0, ...
    )
    ...
    y.pp <- sapply(1:nrow(info$df), function(j) {
      rnorm(
        n = nrow(Xbeta_samps), mean = Xbeta_samps[, j],
        sd = info$df$sd[j]
      )
    })
...
pfit_radon_full <- function(info = radon_1, seed = 32897, ...) {
  ...
  purrr::map(1:length(info$models_mcv), function(mod_no) {
    ...
    iis_vt <- iis_im(loops, Xbeta_samps,
...
pfit_lol <- function(info = lol, seed = 18973, ...) {
  ...
  iis_og <- iis_fm(info$loops, mu_samps,
  ...
  iis_vt <- iis_im(info$loops, Xbeta_samps,
    X = info$X, Y = Y,
    sig_samps = list(
  ...
  list(
    pfit = pfit, phats = phats, P = P, sig_champion = sigs[1], sig_player = sigs[2],
...
pfit_air <- function(info = air$data, seed = 69872, ...) {
  ...
  Sig_inv <- stcarar_sigmainv(
    mean(samples$rho[, "rho.S"]),
    mean(samples$rho[, "rho.T"]),
    info$W, mean(samples$tau2)
  )[-c(1:4), -c(1:4)]
  Sig <- solve(Sig_inv)
