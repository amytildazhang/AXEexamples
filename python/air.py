...
from approxcv.models.examples import AIR
...
modelfolder = "../data-raw/pretrained/air"
savefolder = "./results/air"

dat = hp.load_data(modelfolder)
phi = read_csv("%s/phi.csv" % modelfolder).to_numpy()[:,1]
y = hp.approx_y_poisson(np.log(1/phi), dat['observed'].to_numpy())
cvfolds = dat['IG'].factorize()[0]
J = np.max(cvfolds)
...
offset = np.log(dat.expected.to_numpy())

model = AIR(pm.X, y, W, prior, pm.parnames, integrate = integrate, log_offset = offset)
